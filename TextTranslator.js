...
function Execute(Args){
  try
  {
    ...
    for (var i=0; i<Args.Count; i++) {
      var key = Args.GetKeyAt(i);
      var value = Args.GetValueAt(i);
      argKeys.push(key);
...
function generateDocument() {
  if (inGenerating) {
    ...
    os = fileObject.CreateTextFile(jsFilename, true, false);
    os.write(scriptStr);
    os.close();
  
    // evaluate preprocessed JScript
    os = fileObject.CreateTextFile(outputFilename, true, false);
